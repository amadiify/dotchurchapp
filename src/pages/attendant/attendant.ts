...
export class AttendantPage {
  ...
  getPrograms()
  {
    return this.http.get( this.Myapp.sermon_api + 'getprograms/'+this.churchid+'/'+this.userid).map( res => JSON.parse(res.text().trim()));
  }
  ...
  getMembers(program)
  {
    ...
    if (this.role == 'hod')
    {
        return this.http.get( query + 'departmentMembers/'+program.program+'/'+this.churchid+'/'+this.userid).map(res => res.text());
    }
    else
    {
