...
export class MemberslistPage {
  ...
  attendanceInfo()
  {
    ...
    var query = MyApp.sermon_api + "attendanceInfo/"+program+"/"+this.churchid+"/"+this.userid;
  ...
  allmembers()
  {
    ...
    if (this.role == 'hod')
    {
        return this.http.get( MyApp.sermon_api + 'departmentMembers/'+program+'/'+this.churchid+'/'+this.userid).map(res => res.text());
    }
    else
    {
